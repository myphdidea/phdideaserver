...
class TrustedTimestamps
{
	...
	public static function createRequestfile ($hash)
	{
		...
		$cmd = 'openssl ts -query -digest '.escapeshellarg($hash).' -sha512 -cert -out '.escapeshellarg($outfilepath);
//		putenv('PATH=C:/Program Files/OpenSSL-Win64/bin/');
		putenv('PATH=/usr/bin');
     ...
     */
	public static function signRequestfile ($requestfile_path, $tsa_url,$username_password)
	{
		...
		if ($status != 200 || !strlen($binary_response_string))
			return false;//throw new Exception("The request failed");
	...
	public static function getTimestampFromAnswer ($base64_response_string)
	{
		...
		if ($retcode !== 0)
			return false;//throw new Exception("The reply failed: ".implode(", ", $retarray));
		...
		foreach ($retarray as $retline)
		{
			if (preg_match('~^Time\sstamp\:\s(.*)~', $retline, $matches))
			{
				if(strpos($matches[1],'.'))
				{
					$first_part=substr($matches[1],strpos($matches[1],'.'));
					$response_time = strtotime(str_replace(substr_replace($first_part,'',strpos($first_part,' ')),'',$matches[1]));
				}
				else $response_time = strtotime($matches[1]);
				break;		
			}
		}

		if (!$response_time)
			return false;//throw new Exception("The Timestamp was not found");	
	...
	public static function validate ($hash, $base64_response_string, $response_time, $tsa_cert_file)
	{
		...
		putenv('OPENSSL_CONF=C:/Program Files/OpenSSL-Win64/bin/openssl.cfg');
