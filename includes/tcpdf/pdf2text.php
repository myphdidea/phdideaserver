    ...
    for ($j = 0; $j < count($chars); $j++) {
        ...
        for ($k = 0; $k < $count && $k < count($current); $k++) {
            if (preg_match("#<([0-9a-f]{2,4})>\s+<([0-9a-f]{4,512})>#is", trim($current[$k]), $map))
                $transformations[str_pad($map[1], 4, "0", STR_PAD_LEFT)] = $map[2];
        }
    }
...
//					echo var_dump($transformations);
                }
            }

//			echo $i." transf ".var_dump($transformations);
...
//				echo "again ".var_dump($transformations);
//                echo $textTransformed."<br><br><br>";
